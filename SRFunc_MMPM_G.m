...
% ALL RIGHTS RESERVED.
%% SR Functions for Multiple Mixture Pior Models-Gaussian 
function [RecImage] = SRFunc_MMPM_G(imgs, conf, mix_match)
...
for ii = 1 : length(conf.maxK)
    ...
    for k = 1:size(ModelMeans,2)
        Means_LR = repmat(ModelMeans((patch_size^2 + 1):size(ModelMeans,1),k),1,size(Patches,2));
        Covs_LR = ModelCovs((patch_size^2 + 1):size(ModelCovs, 1), (patch_size^2 + 1):size(ModelCovs,1), k); 
        Respons(k,:) = log(ModelWeights(k)) + loggausspdf(Patches, Means_LR, Covs_LR);   
    end
